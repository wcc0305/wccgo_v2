...
def register():
    ...
    if form.validate_on_submit():
        ...
        send_email(user.email, 'Confirm Your Account',
                   'auth/email/confirm', user=user, token=token)    #此处‘auth/email/confirm’是什么路径？
...
def unconfirmed():
    if current_user.is_anonymous or current_user.confirmed:
        return redirect(url_for('main.index'))
    return render_template('auth/email/unconfirmed.html', Permission=Permission)
...
def password_reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email with instructions to reset your password has been '
              'sent to you.')
        return redirect(url_for('auth.login'))
    return render_template('auth/reset_password.html', form=form, Permission=Permission)
